...
async function deployContracts(deploymentManager: DeploymentManager, deploySpec: DeploySpec): Promise<Deployed> {
  ...
  const USDt = await deploymentManager.existing('USDt', '0x89e8a0f004CC32750b49D0dAbA5a88E88FA090E4', 'sepolia');
  // const WETH = await deploymentManager.existing('WETH', '0x34Dc0CA8c4Be9B5Cc0cCBDA44b988d2a5A88A56b', 'sepolia');
  // const WBTC = await deploymentManager.existing('WBTC', '0x580047e13aD2F0D01C5A53276f51f473BBf37002', 'sepolia');
   const TSTOCK = await deploymentManager.existing(
    'TSTOCK',                              // Alias (must match configuration.json)
    '0xBEae6Fa62362aB593B498692FD09002a9eEd52dc',                   // ← YOUR TOKEN ADDRESS
    'sepolia'                   // Interface to use
  );
